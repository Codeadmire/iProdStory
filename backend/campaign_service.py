...
def generate_campaign(product_id: str, db: Session, model_name: str = "gemini-1.5-flash"):
...
def generate_linkedin_product_page(product_id: str, db: Session, model_name: str = "gemini-1.5-flash"):
    product = db.query(models.Product).filter(models.Product.id == product_id).first()
    if not product:
        return None
