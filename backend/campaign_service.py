...
def generate_campaign(product_id: str, db: Session, model_name: str = "gemini-3.1-pro"):
...
def generate_linkedin_product_page(product_id: str, db: Session, model_name: str = "gemini-3.1-pro"):
    product = db.query(models.Product).filter(models.Product.id == product_id).first()
    if not product:
        return None
